...
python scripts/pixelwise_late_snow_late_summer_ndvi.py \ 
    ...
    --nx 1601 --ny 1081 \
    --downsample 2 \
    --ndvi_doy_start 208 --ndvi_doy_end 274 \
    --sca_doy_start 128 --sca_doy_end 176 \
    --sca_metric mean \
    --out_nc_path pixelwise_late_snow_late_summer_ndvi_full_ds2.nc
...
def _load_annual_sca(
    ...
    elif sca_metric == "last_day_above":
        def _last_day(series_df: pd.DataFrame) -> float:
            valid = series_df.loc[series_df["Snow_16d"] >= sca_threshold, "DOY"]
            if valid.empty:
                return np.nan
            return float(valid.max())

        annual_sca = df.groupby("Year", as_index=True).apply(_last_day).sort_index()
        annual_sca.name = "Snow_16d_last_day_above"
    else:
        raise ValueError(f"Unsupported sca_metric: {sca_metric}")
...
def _quickplot(
    ndvi_mean_raw: np.ndarray,
    r2: np.ndarray,
    ndvi_doy_start: int,
    ndvi_doy_end: int,
    sca_metric: str,
    plot_png_path: Path,
    plot_vmin_ndvi: float,
    plot_vmax_ndvi: float,
    plot_vmin_r2: float,
    plot_vmax_r2: float,
) -> None:
    ...
    axes[0].set_title(f"Mean NDVI (DOY {ndvi_doy_start}–{ndvi_doy_end})")
    ...
    axes[1].set_title(f"Late snow ({sca_metric}) vs late-summer NDVI R²")
...
def run(args: argparse.Namespace) -> None:
    ...
    annual_ndvi = _build_annual_ndvi(ndvi, args.ndvi_doy_start, args.ndvi_doy_end).sortby("year")
    ...
    ndvi_mean_raw = annual_tile.mean("year", skipna=True)
    if args.ndvi_anomaly == "demean_pixel":
        ndvi_longterm_mean = ndvi_mean_raw
        annual_tile_used = annual_tile - ndvi_longterm_mean
    else:
        ndvi_longterm_mean = None
        annual_tile_used = annual_tile

    ndvi_mean_used = annual_tile_used.mean("year", skipna=True)

    sca_annual = _load_annual_sca(
    ...
    ndvi_years = annual_tile_used["year"].values.astype(int)
    ...
    if years.size == 0:
        raise ValueError("No intersected years between NDVI and SCA after filtering.")

    x_all = sca_annual.loc[years].to_numpy(dtype=float)
    finite_x = np.isfinite(x_all)
    years = years[finite_x]
    x = x_all[finite_x]

    if years.size < args.min_n:
        raise ValueError(
            f"Only {years.size} intersected years with finite SCA predictor after filtering; need at least min_n={args.min_n}."
        )

    annual_tile_used = annual_tile_used.sel(year=years)
    y = np.asarray(annual_tile_used.to_numpy(), dtype=float)

    slope, intercept, r2, n = _pixelwise_ols(x=x, y=y, min_n=args.min_n)

    coords = {y_dim: annual_tile_used[y_dim], x_dim: annual_tile_used[x_dim]}
    ...
    data_vars: dict[str, tuple[tuple[str, str], np.ndarray]] = {
        ...
        "r2": ((y_dim, x_dim), r2),
        "n": ((y_dim, x_dim), n),
        "ndvi_mean_raw": ((y_dim, x_dim), np.asarray(ndvi_mean_raw.to_numpy(), dtype=float)),
        "ndvi_mean_used": ((y_dim, x_dim), np.asarray(ndvi_mean_used.to_numpy(), dtype=float)),
    }

    if ndvi_longterm_mean is not None:
        data_vars["ndvi_longterm_mean"] = (
            (y_dim, x_dim),
            np.asarray(ndvi_longterm_mean.to_numpy(), dtype=float),
        )
    ...
    out_ds = xr.Dataset(
        ...
        attrs={
            ...
            "ndvi_anomaly": args.ndvi_anomaly,
            "sca_metric": args.sca_metric,
    ...
    if args.plot_png_path:
        plot_png_path = Path(args.plot_png_path)
    else:
        plot_png_path = out_path.with_name(f"{out_path.stem}_quickplot.png")

    _quickplot(
        ndvi_mean_raw=np.asarray(ndvi_mean_raw.to_numpy(), dtype=float),
        r2=r2,
        ndvi_doy_start=args.ndvi_doy_start,
        ndvi_doy_end=args.ndvi_doy_end,
        sca_metric=args.sca_metric,
        plot_png_path=plot_png_path,
        plot_vmin_ndvi=args.plot_vmin_ndvi,
        plot_vmax_ndvi=args.plot_vmax_ndvi,
        plot_vmin_r2=args.plot_vmin_r2,
        plot_vmax_r2=args.plot_vmax_r2,
    )
    ...
    print("Run summary")
    print(f"  sca_metric: {args.sca_metric}")
    print(f"  ndvi_anomaly: {args.ndvi_anomaly}")
    print(f"  chosen tile x0,y0: ({chosen_x0}, {chosen_y0}), usable_fraction={usable_fraction:.3f}")
    print(f"  intersected years with finite predictor: {years.size}")
    ...
    print(f"  wrote quickplot: {plot_png_path}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pixel-wise late-snow vs late-summer NDVI regression prototype")
    ...
    parser.add_argument("--ndvi_doy_start", type=int, default=220)
    parser.add_argument("--ndvi_doy_end", type=int, default=280)
    parser.add_argument("--ndvi_anomaly", choices=["none", "demean_pixel"], default="none")

    parser.add_argument("--sca_doy_start", type=int, default=140)
    parser.add_argument("--sca_doy_end", type=int, default=200)
    parser.add_argument("--sca_metric", choices=["mean", "last_day_above"], default="mean")
    parser.add_argument("--sca_threshold", type=float, default=0.1)
    ...
    parser.add_argument("--out_nc_path", default="pixelwise_late_snow_late_summer_ndvi.nc")
    parser.add_argument("--plot_png_path", default=None)
    parser.add_argument("--plot_vmin_ndvi", type=float, default=0)
    parser.add_argument("--plot_vmax_ndvi", type=float, default=1)
    parser.add_argument("--plot_vmin_r2", type=float, default=0)
    parser.add_argument("--plot_vmax_r2", type=float, default=1)
